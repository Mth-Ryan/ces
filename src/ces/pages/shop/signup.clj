...
(defn form
  ...
  ([data]
   [:form {:class "space-y-3 w-full"
           :hx-post (str "/crie-sua-conta/partials/form?return-url=" (encode-uri (get data :return-url "/")))
           :hx-swap "outerHTML transition:true"}
    (shared-components/text-input-group-with-auto-error-removal
      {:placeholder "Seu nome"
       :type        "text"
       :name        "firstname"
       :value       (get-in data [:values :firstname] "")
       :errors      (get-in data [:errors :firstname])}
      {:placeholder "Seu sobrenome"
       :type        "text"
       :name        "lastname"
       :value       (get-in data [:values :lastname] "")
       :errors      (get-in data [:errors :lastname])})
    ...
    (shared-components/text-input-with-auto-error-removal
      {:placeholder "Repita sua Senha"
       :type        "password"
       :name        "re-password"
       :value       (get-in data [:values :re-password] "")
       :errors      (get-in data [:errors :re-password])})
    (shared-components/error-messages (get-in data [:top-level :errors]))
    (shared-components/button {:type "submit" :class "w-full"} "Crie sua Conta")]))

(defn content
  [context]
  (list (shared-components/button
    {:hx-replace-url (str "/entrar?return-url=" (get context :return-url "/"))
     :hx-get (str "/entrar/partials/content?return-url=" (get context :return-url "/"))
     :hx-target "#content"
   ...
   [:h1 {:class "font-semibold text-2xl"} "Crie sua conta"]
...
(defn page [context]
  (shop-layouts/auth-layout {:title "Crie sua Conta"} (content context)))
...
(defn content-handler [request]
  (let [return-url (get-in request [:query-params :return-url] "/")]
    (responses/ok (list [:title (make-title :page-title "Crie sua Conta")]
...
(def signup-schema
  {:firstname   [[st/required :message "O nome é obrigatório"]]
   :lastname    [[st/required :message "O sobrenome é obrigatório"]]
   :email       [[st/required :message "O email é obrigatório"]]
   :password    [[st/required :message "A senha é obrigatório"]]
   :re-password [[st/required :message "Repita sua senha"]]})
